...
export default {
	...
	methods: {
		getHeader() {
			axios.options(this.$config.VITE_APP_API_ROUTE+"deployment/actions?package="+this.id, { headers: this.header })
				.then(response => {
		...
		getPackage(reload = false) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"deployment/packages/"+this.id, { headers: this.header })
				.then(response => {
					if(!reload) {
