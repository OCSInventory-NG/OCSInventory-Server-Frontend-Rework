...
export default {
	...
	methods: {
		getHeader() {
			axios.options(this.$config.VITE_APP_API_ROUTE+"automation/scheduler/", { headers: this.header })
				.then(response => {
		...
		getSchedulers() {
			axios.get(this.$config.VITE_APP_API_ROUTE+"automation/scheduler/", { headers: this.header })
				.then(response => {
