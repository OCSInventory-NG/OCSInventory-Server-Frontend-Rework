...
export default {
	...
	methods: {
		getTriggerModels() {
			axios.get(this.$config.VITE_APP_API_ROUTE+"automation/triggers/", { headers: this.header })
				.then(response => {
		...
		getRuleInfo() {
			axios.get(this.$config.VITE_APP_API_ROUTE+"automation/rule/"+this.id, { headers: this.header })
				.then(response => {
