...
export default {
	...
	methods: {
		...
		getAccountinfo(id) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"accountinfo/config/"+id+"/", { headers: this.header })
				.then(response => {
		...
		onSubmit(event) {
			...
			if(!this.update) {
				axios.post(this.$config.VITE_APP_API_ROUTE+"accountinfo/config/", this.row, { headers: this.header })
					.then(() => {
				...
				axios.patch(this.$config.VITE_APP_API_ROUTE+"accountinfo/config/"+this.row.id+"/", update,
					{ headers: this.header })
					.then(() => {
						this.createwithsuccess = true
						this.createerrormsg = null
