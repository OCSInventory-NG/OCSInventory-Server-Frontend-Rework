...
export default {
	...
	methods: {
		...
		getMappingConfig(id) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"auth_mapping?auth_config="+id, { headers: this.header })
				.then(response => {
		...
		onSubmit(event) {
			...
			if(jsonToUpdate.length) {
				jsonToUpdate.forEach(data => {
					axios.patch(this.$config.VITE_APP_API_ROUTE+"auth_mapping/"+data.id+"/", data, { headers: this.header })
						.then(() => {
			...
			if(jsonToAdd.length) {
				axios.post(this.$config.VITE_APP_API_ROUTE+"auth_mapping/", jsonToAdd, { headers: this.header })
					.then(() => {
						this.createwithsuccess = true
						this.createerrormsg = null
						this.createerror = false
			...
			if(jsonToDelete.length) {
				jsonToDelete.forEach(data => {
					axios.delete(this.$config.VITE_APP_API_ROUTE+"auth_mapping/"+data.id+"/", { headers: this.header })
						.then(() => {
							this.createwithsuccess = true
							this.createerrormsg = null
							this.createerror = false
