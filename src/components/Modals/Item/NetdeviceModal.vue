...
export default {
	...
	methods: {
		...
		getNetdevice(id) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"netdevices/"+id+"/", { headers: this.header })
				.then(response => {
		...
		onSubmit(event) {
			...
			axios.patch(this.$config.VITE_APP_API_ROUTE+"netdevices/"+this.row.id+"/", this.row, { headers: this.header })
				.then(() => {
