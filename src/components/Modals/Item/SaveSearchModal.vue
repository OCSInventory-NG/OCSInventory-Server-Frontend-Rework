...
export default {
	...
	methods: {
		...
		getSavedSearch(id) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"search/save/"+id, { headers: this.header })
				.then(response => {
		...
		onSubmit(event) {
			...
			if(!this.update) {
				...
				if(this.searchaction == "create") {
					axios.post(this.$config.VITE_APP_API_ROUTE+"search/save/", this.savesearch, { headers: this.header })
						.then(() => {
					...
					axios.patch(this.$config.VITE_APP_API_ROUTE+"search/save/"+this.savesearch.id+"/", this.savesearch, 
						{ headers: this.header })
						.then(() => {
							this.createwithsuccess = true
							this.createerrormsg = null
