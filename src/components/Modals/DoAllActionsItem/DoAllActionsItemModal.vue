...
export default {
	...
	methods: {
		...
		onSubmit(event) {
			...
			if(jsonAdd.length > 0) {
				axios.post(this.$config.VITE_APP_API_ROUTE+this.route+"/", jsonAdd, { headers: this.header })
					.then(() => {
						this.errormsg = null
						this.errored = false
					})
		...
		onUpdate(input) {
			if(input.id) {
				axios.patch(this.$config.VITE_APP_API_ROUTE+this.route+"/"+input.id+"/", input, 
					{ headers: this.header })
					.then(() => {
						this.errormsg = null
						this.errored = false
		...
		getData() {
			...
			axios.get(this.$config.VITE_APP_API_ROUTE+this.route+"/?"+this.get, { headers: this.header })
				.then(response => {
		...
		removeField(index, fieldType) {
			if(typeof fieldType[index].id !== 'undefined') {
				axios.delete(this.$config.VITE_APP_API_ROUTE+this.route+"/"+fieldType[index].id, { headers: this.header })
					.then(() => {
						this.errormsg = null
						this.errored = false
					})
