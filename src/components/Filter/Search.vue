	...
	methods: {
		...
		getFields(route, masterindex, index, section = false) {
			...
			if(route == "accountinfo/config?datatarget=ASSET") {
				...
				axios.get(this.$config.VITE_APP_API_ROUTE+"fields?section="+route, { headers: this.header })
					.then(response => {
						this.loading = true
		...
		getSections(templateId, masterindex, index) {
			axios.get(this.$config.VITE_APP_API_ROUTE+"sections?template="+templateId, { headers: this.header })
				.then(response => {
