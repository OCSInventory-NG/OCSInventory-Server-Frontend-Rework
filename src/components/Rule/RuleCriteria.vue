...
export default {
	...
	methods: {
		getModelField() {
			axios.options(this.$config.VITE_APP_API_ROUTE+this.triggermodel[this.trigger].route, { headers: this.header })
				.then(response => {
		...
		onSubmit(event) {
			...
			axios.patch(this.$config.VITE_APP_API_ROUTE+"automation/rule/"+this.id+"/", this.logicupdate, 
				{ headers: this.header })
				.then(() => {
